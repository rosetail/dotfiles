...
;; fix error due to renamed variable in emacs 29
...
(use-package emacs
  ...
  ;; don't prompt to save passwords in authinfo
...
(use-package evil-collection
  ...
  :config
  ;; emms bindings are all messed up
  ;; (delete 'emms evil-collection-mode-list)
...
**** extensions
***** COMMENT don't preview tramp buffers
#+begin_src emacs-lisp :tangle yes
...
(use-package em-term
  ...
  (add-to-list 'eshell-visual-commands "nmtui-connect"))
#+end_src
**** autosuggestions
#+begin_src emacs-lisp :tangle yes
...
*** dtache
 ...
 (use-package dtache
   ...
   (setq dtache-detach-key (kbd "C-\\")
         ...
         ;; this has to query $HOME because otherwise dtache looks in /root for sudo commands
         dtache-env (concat (getenv "HOME") "/.emacs.d/var/dtache/dtache-env")
         ;; obey no-littering
...
*** toggle sudo
#+begin_src emacs-lisp :tangle yes
;; TODO: fix this so sudo can be toggled off
;; also try wrapping it in save-window-excursion or something
...
*** emms
#+begin_src emacs-lisp :tangle yes
;; TODO: make cover art actually be cached, sort albums by name, and start on album page
;; TODO: setup stream mode bindings and maybe make a kcsm stream
...
(use-package emms
  ...
  ;; use find instead of builtin alternatives to find music files
  ...
  ;; (setq emms-browser-album-sort-function #'emms-browser-sort-by-name)
  ...
  (add-to-list 'emms-info-functions #'emms-info-mpd t)
  :hook
  (emms-browser-mode
   . (lambda (&rest _)
       (general-define-key
        ...
        :states 'normal
        "n" 'emms-browser-next-non-track
        "e" 'emms-browser-prev-non-track
        "C-n" 'evil-next-visual-line
        "C-e" 'evil-previous-visual-line
        "N" 'evil-next-visual-line
        "E" 'evil-previous-visual-line
...
(use-package emms-tag-editor
  ...
  (setq emms-tag-editor-rename-format "%02n - %t")
  
  ;; add discnumber to tag editor
  (setq emms-tag-editor-tags
        '((info-artist      . "a")
          ...
          (info-note        . "c")))
  :general
  (:keymaps 'emms-tag-editor-mode-map
            "C-c C-k" 'quit-window))
#+end_src
...
(use-package emms-mark
  :straight nil
  :init
  ;; use normal mode for playlist buffer
  (evil-set-initial-state 'emms-mark-mode 'normal)
  (evil-collection-inhibit-insert-state 'emms-mark-mode-map)
  ...
  :hook
  ;; setting `emms-player-default-major-mode' doesn't work so we need a hook
  (emms-playlist-mode . emms-mark-mode)
  (evil-collection-setup
   . (lambda (&rest _)
       (general-define-key
        ...
        'emms-mark-mode-map
        :states 'normal
        "n" (lambda () (interactive) (emms-playlist-next))
        "e" (lambda () (interactive) (emms-playlist-previous))
        ...
        ;; sorting
        "sn" #'emms-playlist-sort-by-natural-order
        "sa" #'emms-playlist-sort-by-info-artist
        "sc" 'emms-playlist-sort-by-play-count
        "sb" 'emms-playlist-sort-by-info-album
        "sl" 'emms-playlist-sort-by-last-played
        "st" 'emms-playlist-sort-by-info-title
        "se" 'emms-playlist-sort-by-file-extension

        "sp" 'emms-playlist-sort-by-info-performer
        "sy" 'emms-playlist-sort-by-info-year
        "so" 'emms-playlist-sort-by-info-note
        "sC" 'emms-playlist-sort-by-info-composer
        "sL" 'emms-playlist-sort-by-list
        "sN" 'emms-playlist-sort-by-name
        "sT" 'emms-playlist-sort-by-file-mtime
        ...
        "W" 'emms-mark-copy-marked-tracks

        "m" 'emms-mark-forward

        ;; either unmark track or undo
        "u" (lambda ()
              (interactive)
              (if (emms-mark-has-markedp)
                  (call-interactively #'emms-mark-unmark-forward)
                (emms-playlist-mode-undo)))
        ;; bind C-/ to undo so we can still access it when there are marks
...
*** mpd
#+begin_src emacs-lisp :tangle yes
;; TODO: make emms add songs to mpd playlist more often
...
*** hydra
#+begin_src emacs-lisp :tangle yes
;; TODO: support metaplaylist/multiple playlist management

(defhydra hydra-emms (:hint nil)
  "
%s(progn (require 'emms) (if emms-player-playing-p (format emms-show-format (emms-track-description (emms-playlist-current-selected-track))) \"Nothing playing right now\"))
^-^-^-^----------------^^^^----------------------^^-------------------
^ ^ ^Playlists^          ^^^Playback^              ^Seeking^
^-^-^-^----------------^^^^----------------------^^-------------------
^ ^ _s_: Smart Browse       ^^_SPC_: Play/Pause       _i_: +10s
^ ^ _p_: Playlist         ^^_M-SPC_: Play/Pause       _h_: -10s
^ ^ _b_: Browser            _<_/_>_: Next/Previous    _n_: +1m
_a_/_A_: Play/Add File        ^^_x_: Stop             _e_: -1m
^ ^ _u_: Update MPD DB
"
  ("s" emms-smart-browse :color blue)
  ("p" emms :color blue)
  ("b" emms-browser :color blue)
  ("a" emms-play-file :color blue)
  ("A" emms-add-file :color blue)
  ("u" (lambda () (interactive) (emms-player-mpd-update-all) (emms-cache-set-from-mpd-all)) :color blue)

  ("SPC" emms-pause :color blue)
  ("M-SPC" emms-pause)
  ("<"   emms-previous)
  (">"   emms-next)
  ("x"   emms-stop)
  ...
  ("n" (lambda () (interactive) (emms-seek 60)))
  ("e" (lambda () (interactive) (emms-seek -60))))

(my/add-to-global-hydra '("e" hydra-emms/body "Emms" :column "Misc"))
#+end_src
